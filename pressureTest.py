            ...
            if self.protocol == "tcp":
                ...
                if not self.connected:
                    ...
                    self.context.load_cert_chain(
                        certfile=self.crtfile, keyfile=self.keyfile
                    )
                    self.context.check_hostname = False
                    self.context.verify_mode = ssl.CERT_NONE
                    self.connection = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    self.ssl_sock = self.context.wrap_socket(
                        self.connection, server_side=False
                    )
...
def decode_n0(response_to_decode: bytes, head_len: int):
    ...
    nothing
    """
    response_to_decode_str, msg_len, str_pointer = common_parser(
        response_to_decode, head_len
    )
...
def decode_no(response_to_decode: bytes, head_len: int):
    ...
    response_to_decode, msg_len, str_pointer = common_parser(
        response_to_decode, head_len
    )
    if response_to_decode[str_pointer : str_pointer + 2] == "00":  # No errors
        if len(response_to_decode) >= (24 + head_len):  # Mode 00
            ...
            print(
                "I/O buffer size: ",
                BUFFER_SIZE.get(
                    response_to_decode[str_pointer : str_pointer + 1], "Unknown"
                ),
            )
            ...
            print(
                ...
                response_to_decode[str_pointer : str_pointer + socket_field_len],
            )
            str_pointer = str_pointer + socket_field_len
            print(
                "Firmware number: ", response_to_decode[str_pointer : str_pointer + 9]
            )
            ...
            if response_to_decode[str_pointer : str_pointer + 1] == "0":
                ...
                print(
                    "All security settings relevant to PCI HSM compliance have compliant values."
                )
...
def decode_ni(response_to_decode: bytes, head_len: int):
    ...
    if response_to_decode[str_pointer : str_pointer + 2] == "00":  # No errors
        ...
        for record in range(records_to_follow):
            ...
            str_pointer = str_pointer + 1
            print(
                "Local port number: ", response_to_decode[str_pointer : str_pointer + 4]
            )
            str_pointer = str_pointer + 4
            print(
                "IP Address: ",
                hex2ip(response_to_decode[str_pointer : str_pointer + 8]),
            )
            str_pointer = str_pointer + 8
            print(
                "Remote port number: ",
                response_to_decode[str_pointer : str_pointer + 4],
            )
            str_pointer = str_pointer + 4
            print(
                "Connection Status: ",
                NET_CONNECTION_STATUS.get(
                    response_to_decode[str_pointer : str_pointer + 1], "Reserved"
                ),
            )
            str_pointer = str_pointer + 1
        ...
        print(
            "Total Bytes Sent: ",
            int(response_to_decode[str_pointer : str_pointer + 16], 16),
        )
        ...
        print(
            ...
            int(response_to_decode[str_pointer : str_pointer + 8], 16),
        )
        str_pointer = str_pointer + 8
        print(
        ...
        str_pointer = str_pointer + 8
        print(
            "Total Packets Discarded During Send: ",
            int(response_to_decode[str_pointer : str_pointer + 8], 16),
        )
        str_pointer = str_pointer + 8
        print(
            "Total Packets Discarded During Receive: ",
            int(response_to_decode[str_pointer : str_pointer + 8], 16),
        )
        str_pointer = str_pointer + 8
        print(
        ...
        if (
            SPECIFIC_ERROR.get(response_to_decode[str_pointer : str_pointer + 2])
            is not None
        ):
            print(
...
def decode_j8(response_to_decode: bytes, head_len: int):
    ...
    if response_to_decode[str_pointer : str_pointer + 2] == "00":
        ...
        str_pointer = str_pointer + 10
        print(
            "Pin verifies/minute: ", response_to_decode[str_pointer : str_pointer + 7]
        )
...
def decode_j2(response_to_decode: bytes, head_len: int):
    ...
    if response_to_decode[str_pointer : str_pointer + 2] == "00":
        ...
        while (str_pointer + 15) <= msg_len:
            print(
                "Starting percentage: ",
                response_to_decode[str_pointer : str_pointer + 3],
            )
            str_pointer = str_pointer + 3
            print(
                "Ending percentage: ", response_to_decode[str_pointer : str_pointer + 3]
            )
            str_pointer = str_pointer + 3
            print(
                "Number Times Periods: ",
                response_to_decode[str_pointer : str_pointer + 10],
            )
            str_pointer = str_pointer + 10
...
def decode_jk(response_to_decode: bytes, head_len: int):
    ...
    TAMPER_STATUS_CODE = {"0": "Unknown", "1": "Not Tampered", "2": "Tampered"}
    ...
    LMK_ALGORITHM_CODE = {"0": "3DES2Key", "1": "3DES3Key", "2": "AES 256-bit"}
    LMK_SCHEME_CODE = {"V": "Variant", "K": "Keyblock"}
    LMK_STATUS_CODE = {"L": "Live", "T": "Test"}
    LMK_AUTH_CODE = {"0": "Not authorized", "1": "Authorized"}
    FRAUD_CODE = {"0": "not exceeded (or not enabled)", "1": "exceeded"}
    response_to_decode, msg_len, str_pointer = common_parser(
        response_to_decode, head_len
    )
    if response_to_decode[str_pointer : str_pointer + 2] == "00":
        ...
        print(
            "Console State: ",
            CONSOLE_STATUS_CODE.get(
                response_to_decode[str_pointer : str_pointer + 1], "?"
            ),
        )
        str_pointer = str_pointer + 1
        print(
            "payShield Manager State: ",
            CONSOLE_STATUS_CODE.get(
                response_to_decode[str_pointer : str_pointer + 1], "?"
            ),
        )
        ...
        if tamper_state == "2":
            print(
                "Tamper Cause: ",
                TAMPER_CAUSE_CODE.get(
                    response_to_decode[str_pointer : str_pointer + 2], "?"
                ),
            )
            str_pointer = str_pointer + 2
            ...
            for lmk in lmks_array:
                if len(lmk) > 0:
                    ...
                    print(
                        "Authorised: ",
                        LMK_AUTH_CODE.get(
                            lmk[local_lmk_pointer : local_lmk_pointer + 1], "?"
                        ),
                    )
                    local_lmk_pointer = local_lmk_pointer + 1
                    print(
                        "Num Authorised Activities: ",
                        lmk[local_lmk_pointer : local_lmk_pointer + 2],
                    )
                    local_lmk_pointer = local_lmk_pointer + 2
                    print(
                        "LMK Scheme: ",
...
def decode_ecc(response_to_decode: bytes, head_len: int):
    ...
    if response_to_decode_str[str_pointer : str_pointer + 2] == "00":
        ...
        print(
            ...
            response_to_decode[
                str_pointer + key_len : str_pointer + key_len + 1
            ].decode("ascii", "ignore"),
        )
...
def check_returned_command_verb(
    result_returned: bytes, head_len: int, command_sent: str
) -> Tuple[int, str, str]:
    """
...
def check_return_message(result_returned: bytes, head_len: int) -> Tuple[str, str]:
    if (
        len(result_returned) < 2 + head_len + 2
    ):  # 2 bytes for len + 2 header len + 2 for command
        return "ZZ", "Incomplete message"
    # decode the first two bytes returned and transform them in integer
    try:
        expected_msg_len = int.from_bytes(
            result_returned[:2], byteorder="big", signed=False
        )
    except ValueError:
...
def run_test(
    ...
    try:
        ...
        if return_code_tuple[0] != "ZZ":
            print()
            check_result_tuple = check_returned_command_verb(
                data, header_len, host_command
            )

        print("Return code: " + str(return_code_tuple[0]) + " " + return_code_tuple[1])
        if check_result_tuple[0] != 0:
            print(
                "NOTE: The response received from the HSM seems unrelated to the request!"
            )
...
if __name__ == "__main__":
    print(
        "PayShield stress utility, version "
        + VERSION
        + ", by Marco S. Zuppone - msz@msz.eu - https://msz.eu"
    )
    ...
    group.add_argument(
        "--key",
        help="RSA key length. Accepted values are between 320 and 4096.",
        type=int,
    )
    group.add_argument("--nc", help="Perform a NC test.", action="store_true")
    group.add_argument(
        "--no",
        help="Retrieve HSM status information using NO command.",
        action="store_true",
    )
    group.add_argument(
        "--ni",
        help="Return information about the Ethernet Host port 1.",
        action="store_true",
    )
    group.add_argument(
        "--pci",
        help="Check if the HSM is set in PCI compliant mode.",
        action="store_true",
    )
    group.add_argument(
        "--j2", help="Get HSM Loading using J2 command.", action="store_true"
    )
    group.add_argument(
        "--j4", help="Get Host Command Volumes using J4 command.", action="store_true"
    )
    group.add_argument(
        "--j8",
    ...
    group.add_argument(
        "--b2", help="Echo received data back to the user.", action="store_true"
    )
    group.add_argument(
        "--randgen", help="Generate a random value 8 bytes long.", action="store_true"
    )
    group.add_argument(
        "--ecc",
    ...
    parser.add_argument(
        "--key-use",
        help="Select the key mode of use.",
        default="S",
        type=str.upper,
        choices=["S", "X", "N"],
    )
    parser.add_argument(
        "--key-exportability",
        help="Select the key exportability.",
        default="S",
        type=str.upper,
        choices=["N", "E", "S"],
    )
    ...
    parser.add_argument(
        "--forever",
        help="If this option is specified the program runs forever.",
        action="store_true",
    )
    parser.add_argument(
        "--decode",
        help="If specified the reply of the payShield is interpreted "
        "if a decoder function for that command has been implemented.",
        action="store_true",
    )
    parser.add_argument(
        "--times",
        help="How many times to repeat the operation "
    ...
    parser.add_argument(
        "--proto",
        help="Accepted values are tcp, udp or tls. The default is tcp",
        default="tcp",
        choices=["tcp", "udp", "tls"],
        type=str.lower,
    )
    parser.add_argument(
        "--keyfile",
        help="Client key file, used if the protocol is TLS.",
        type=Path,
        default="client.key",
    )
    parser.add_argument(
        "--crtfile",
        help="Client certificate file, used if the protocol is TLS.",
        type=Path,
        default="client.crt",
    )
    parser.add_argument(
        "--echo",
        help="Payload sent using the echo command B2.",
        type=str,
        default="PayShieldStress Echo Test",
        action="store",
    )
    ...
    if len(command) == 0:
        print(
            "You forgot to specify the action you want to to perform on the payShield"
        )
        sys.exit()
    if args.proto == "tls":
        ...
        if args.port < 2500:
            print(
                ...
                args.port,
                " please check that you passed the right value to the "
                "--port parameter",
            )
    if args.proto == "tls":
        payConnInst = PayConnector(
            args.host, args.port, args.proto, args.keyfile, args.crtfile
        )
    else:
        ...
        while True:
            ...
            if args.decode:
                run_test(
                    ...
                    len(args.header),
                    DECODERS.get(
                        command[len(args.header) : len(args.header) + 2], None
                    ),
                )
            else:
